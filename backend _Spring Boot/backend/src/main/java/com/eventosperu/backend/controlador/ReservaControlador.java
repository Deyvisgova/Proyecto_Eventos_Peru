...
public class ReservaControlador {
    ...
    public Reserva confirmar(@PathVariable Integer id) {
        ...
        String detalle = detalleReservaRepositorio.findByReserva(reserva).stream()
                .map(d -> {
                    String nombreOpcion = d.getOpcion() != null ? d.getOpcion().getNombreOpcion() : "Servicio reservado";
                    return String.format("- %s (cant: %d) S/ %.2f", nombreOpcion, d.getCantidad(), d.getPrecioUnitario());
                })
