...
export class ReservasProveedor implements OnInit {
  ...
  private construirMensajeConfirmacion(reserva: Reserva): string {
    ...
    const nombreEvento = detallesReserva[0]?.servicio?.evento?.nombreEvento;

    const bloqueServicios = detallesReserva
      .map((d) => {
        const opciones = d.servicio.descripcion ? ` (${d.servicio.descripcion})` : '';
        const subtotal = (d.precioUnitario || 0) * (d.cantidad || 0);
        return `• ${d.servicio.nombreServicio}${opciones} x${d.cantidad} - S/ ${subtotal.toFixed(2)}`;
      })
      .join('\n');
