  ...
  construirCuerpoCorreo(reserva: Reserva, det: DetalleReserva[] = []) {
    const servicios = det.length
      ? det
          .map(
            (d) =>
              `• ${d.opcion?.nombreOpcion || 'Servicio reservado'} (${d.cantidad} x S/ ${
    ...
    const nombreEvento =
      det.find((d) => d.opcion?.proveedorServicio?.catalogoServicio?.evento?.nombreEvento)
...
Gracias por confiar en nosotros.`;
  ...
  private formatearFechaLegible(fecha: string | Date | undefined | null) {
    ...
    const valor = fecha instanceof Date ? fecha : `${fecha}`;
