...
export class EventoCliente implements OnInit {
  ...
  private logoFallback = 'assets/servicios/servicio-generico.svg';
  private logosOcultos = new Set<number>();
  ...
  logoProveedor(p: ProveedorServicio) {
    if (this.logosOcultos.has(p.idProveedorServicio)) return '';
  ...
  vistaPreviaOpciones(p: ProveedorServicio): string[] {
    const opciones = this.opcionesPorProveedor[p.idProveedorServicio] || [];
    return opciones.slice(0, 3).map((o) => o.nombreOpcion);
  }

  opcionesRestantes(p: ProveedorServicio): number {
    const opciones = this.opcionesPorProveedor[p.idProveedorServicio] || [];
    return Math.max(opciones.length - 3, 0);
  }

  ocultarLogo(p: ProveedorServicio, event?: Event) {
    this.logosOcultos.add(p.idProveedorServicio);
  ...
  urlImagenOpcion(opcion: ServicioOpcion) {
    ...
    const catalogoPlano = opcion.proveedorServicio?.catalogoServicio as
      | (Record<string, unknown> & { evento?: Evento })
      | undefined;
    const proveedorServicioPlano = opcion.proveedorServicio as Record<string, unknown> | undefined;
    const posiblesRutas = [
      opcion.urlFoto,
      opcionPlano['foto'],
      opcionPlano['imagen'],
      ...
      catalogoPlano?.['urlFoto'],
      catalogoPlano?.['foto'],
      proveedorServicioPlano?.['urlFoto'],
    ].filter((v): v is string => typeof v === 'string' && !!v.trim());

    const rutaElegida = posiblesRutas[0];
    const rutaResuelta = this.resolverRutaImagen(rutaElegida);
    return rutaResuelta || 'assets/servicios/servicio-generico.svg';
  }

  fotoGenerica(event: Event) {
    const target = event.target as HTMLImageElement | null;
    if (target) {
      target.src = 'assets/servicios/servicio-generico.svg';
    }
  }
